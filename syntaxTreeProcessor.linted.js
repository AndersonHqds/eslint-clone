export default class SyntaxTreeProcessor {
  ...
  #handleExpressionStatement(node) {
    ...
    this.#storeError(this.#messages.useLet(originalKind), nodeDeclaration.loc.start);
  ...
  #checkDeclartionsThatNeverChanged() {
    [...this.#variables.values()].filter(({stage, nodeDeclaration}) => stage === this.#stages.declaration && nodeDeclaration.kind !== this.#variableKinds.const).forEach(({nodeDeclaration}) => {
  ...
  process(ast) {
    this.#traverse(ast);
    this.#checkDeclartionsThatNeverChanged();
